...
TENSOR_COLS = ["embeddings_name5"]

DATA_SPEC_TENSOR_COLS = {
    "embeddings_name0": (0, 1, np.float64),
    "embeddings_name1": (0, 1, np.float64),
    "embeddings_name2": (0, 1, np.float64),
    "embeddings_name3": (0, 1, np.float64),
    "embeddings_name4": (0, 1, np.float64),
    TENSOR_COLS[0]: (None, 8, (0, 1, np.float64)),
    "one_hot0": (0, 3, np.int64),
    "one_hot1": (0, 50, np.int64),
    "labels": (0, 1, np.float64),
}
...
def generate_file(file_index, global_row_index, num_rows_in_file,
                  num_row_groups_per_file, data_dir, data_spec):
    # TODO(Clark): Generate skewed row groups according to max_row_group_skew.
...
def generate_row_group(group_index, global_row_index, num_rows_in_group,
    ...
    for col, (low, high, dtype) in data_spec.items():
        if isinstance(dtype, tuple):
            shape = (num_rows_in_group, high)
            low, high, dtype = dtype
            serialize = True
        else:
            shape = num_rows_in_group
            serialize = False
        if dtype in (np.int16, np.int32, np.int64):
            buffer[col] = np.random.randint(low, high, shape, dtype=dtype)
        elif dtype in (np.float32, np.float64):
            buffer[col] = ((high - low) * np.random.rand(shape) + low)
        if serialize:
            buffer[col] = [item.tobytes() for item in buffer[col]]
