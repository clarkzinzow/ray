...
/// handling a request and sends the final reply.
...
/// Represents the callback function to be called when a server-side streaming
/// `ServiceHandler` sends a streaming reply, and maybe finishes handling the request.
/// \param status The status would be returned to client.
/// \param success Success callback which will be invoked when the reply is successfully
/// sent to the client.
/// \param failure Failure callback which will be invoked when the reply fails to be
/// sent to the client.
/// \param finished Whether the RPC is finished or not.
using SendStreamingReplyCallback =
    std::function<void(Status status, std::function<void()> success,
                       std::function<void()> failure, bool finished)>;

/// Represents the callback function to be called when a server-side streaming
/// `ServiceHandler` finishes handling a request. \param status The status would be
/// returned to client. \param success Success callback which will be invoked when the
/// reply is successfully sent to the client. \param failure Failure callback which will
/// be invoked when the reply fails to be sent to the client. \param finished Whether the
/// RPC is finished or not.
using SendFinishedCallback = std::function<void(
...
enum class ServerCallState {
  ...
  PROCESSING,
  /// Request processing is not done, and reply is being sent to client.
  /// This should only be used by server-side streaming RPCs.
  SENDING_REPLY,
...
/// Represents the generic signature of a server-side streaming
/// `FooServiceHandler::HandleBar()` function, where `Foo` is the service name and `Bar`
/// is the rpc method name.
///
/// \tparam ServiceHandler Type of the handler that handles the request.
/// \tparam Request Type of the request message.
/// \tparam Reply Type of the reply message.
template <class ServiceHandler, class Request, class Reply>
using HandleRequestFunctionStreaming = void (ServiceHandler::*)(
    const Request &, Reply *, SendStreamingReplyCallback, SendFinishedCallback);
...
class ServerCallImpl : public ServerCall {
 ...
 private:
  ...
  void SendReply(const Status &status) {
    state_ = ServerCallState::FINISHED;
...
/// Implementation of `ServerCall` for server-side streaming RPCs.
/// It represents `ServerCall` for a particular server-side streaming RPC method.
///
/// \tparam ServiceHandler Type of the handler that handles the request.
/// \tparam Request Type of the request message.
/// \tparam Reply Type of the reply message.
template <class ServiceHandler, class Request, class Reply>
class StreamingServerCallImpl : public ServerCall {
 public:
  /// Constructor.
  ///
  /// \param[in] factory The factory which created this call.
  ...
  /// \param[in] handle_request_function Pointer to the service handler function.
  /// \param[in] io_service The event loop.
  StreamingServerCallImpl(const StreamingServerCallFactory &factory,
                          ServiceHandler &service_handler,
                          HandleRequestFunctionStreaming<ServiceHandler, Request, Reply>
  ...
  void HandleRequest() override {
    if (!io_service_.stopped()) {
      ...
      // to send reply and remove it from cq
      RAY_LOG(DEBUG) << "Handle service has been closed.";
      SendReply(Status::Invalid("HandleServiceClosed"), true);
    }
  }
  ...
  void HandleRequestImpl() {
    ...
    (service_handler_.*handle_request_function_)(
        request_, &reply_,
        [this](Status status, std::function<void()> success,
               std::function<void()> failure, bool finish = false) {
          // These two callbacks must be set before `SendReply`, because `SendReply`
          // is async and this `ServerCall` might be deleted right after `SendReply`.
          send_reply_success_callback_ = std::move(success);
          send_reply_failure_callback_ = std::move(failure);

          // When the handler is done with the request, send the reply.
          // Must send reply at the bottom of this callback, once we invoke this funciton,
          // this server call might be deleted
          SendReply(status, finish);
        },
        [this](Status status, std::function<void()> success,
               std::function<void()> failure) {
          // These two callbacks must be set before `SendFinished`, because
          // `SendFinished` is async and this `ServerCall` might be deleted right
          // after `SendFinished`.
          send_reply_success_callback_ = std::move(success);
          send_reply_failure_callback_ = std::move(failure);
          SendFinished(status);
 ...
 private:
  /// Tell gRPC to send reply asynchronously and maybe finished the request..
  void SendReply(const Status &status, bool finish) {
    if (finish) {
      state_ = ServerCallState::FINISHED;
      stream_writer_.WriteAndFinish(reply_, grpc_impl::WriteOptions(),
                                    RayStatusToGrpcStatus(status), this);
    } else {
      state_ = ServerCallState::SENDING_REPLY;
      stream_writer_.Write(reply_, this);
    }
  }

  /// Tell gRPC to finish this request.
  void SendFinished(const Status &status) {
    state_ = ServerCallState::FINISHED;
    stream_writer_.Finish(RayStatusToGrpcStatus(status), this);
  }
  ...
  /// Pointer to the service handler function.
  HandleRequestFunctionStreaming<ServiceHandler, Request, Reply> handle_request_function_;
  ...
  /// The stream writer.
  grpc_impl::ServerAsyncWriter<Reply> stream_writer_;
...
/// \tparam Reply Type of the reply message.
template <class GrpcService, class Request, class Reply>
using RequestCallFunctionStreaming = void (GrpcService::AsyncService::*)(
    grpc::ServerContext *, Request *, grpc_impl::ServerAsyncWriter<Reply> *,
    grpc::CompletionQueue *, grpc::ServerCompletionQueue *, void *);
...
/// \tparam Reply Type of the reply message.
template <class GrpcService, class ServiceHandler, class Request, class Reply>
class StreamingServerCallFactoryImpl : public ServerCallFactory {
  using AsyncService = typename GrpcService::AsyncService;
 ...
 public:
  ...
  /// \param[in] cq The `CompletionQueue`.
  /// \param[in] io_service The event loop.
  StreamingServerCallFactoryImpl(
      AsyncService &service,
      RequestCallFunctionStreaming<GrpcService, Request, Reply> request_call_function,
      ServiceHandler &service_handler,
      HandleRequestFunctionStreaming<ServiceHandler, Request, Reply>
          handle_request_function,
      const std::unique_ptr<grpc::ServerCompletionQueue> &cq,
  ...
  void CreateCall() const override {
    // Create a new `ServerCall`. This object will eventually be deleted by
    // `GrpcServer::PollEventsFromCompletionQueue`.
    auto call = new StreamingServerCallImpl<ServiceHandler, Request, Reply>(
        *this, service_handler_, handle_request_function_, io_service_);
    /// Request gRPC runtime to starting accepting this kind of request, using the call as
    /// the tag.
    (service_.*request_call_function_)(&call->context_, &call->request_,
                                       &call->stream_writer_, cq_.get(), cq_.get(), call);
  }
 ...
 private:
  ...
  /// Pointer to the `AsyncService::RequestMethod` function.
  RequestCallFunctionStreaming<GrpcService, Request, Reply> request_call_function_;
